...
export class NombaClient {
  ...
  private async ensureToken(): Promise<string> {
    ...
    try {
      ...
      functions.logger.error('[NOMBA] Failed to obtain access token', error);
      throw error;
    }
  }
  ...
  validateWebhookSignature(payload: string, signature: string): boolean {
    if (!this.config.webhookSecret) {
      functions.logger.error('[NOMBA] Webhook secret not configured');
      return false;
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (statusCode && statusCode >= 400 && statusCode < 500 && statusCode !== 429) {
          functions.logger.error(`[NOMBA] Non-retryable error on ${operationName}`, {
            statusCode,
        ...
        if (attempt < maxRetries) {
          ...
          functions.logger.warn(`[NOMBA] Retrying ${operationName} after ${delay}ms (attempt ${attempt}/${maxRetries})`);
