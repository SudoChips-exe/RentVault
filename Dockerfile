...
# Supabase Storage client config - same "not secret" reasoning as the
# Firebase block above (this is the public anon key, meant to ship to
# browsers); also baked in at build time since dashboards' Next.js
# prerenders pages that import lib/supabase.ts even for 'use client' pages.
...
FROM node:22-bookworm-slim AS dashboards-build
ARG FIREBASE_API_KEY
ARG FIREBASE_AUTH_DOMAIN
ARG FIREBASE_PROJECT_ID
ARG FIREBASE_STORAGE_BUCKET
