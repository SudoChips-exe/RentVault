...
APIConf Hackathon — Monnify Integrations Track
...
1. **Tenant pays in full**, choosing either Monnify or Nomba checkout at the moment of payment, into a held state. The money is not in the landlord's account and not in RentVault's general accounts — it sits in an explicitly tracked, auditable held state tied to that specific transaction.
...
4. **On approval**, the same provider that collected the payment automatically splits the held funds according to a configurable percentage split — landlord, agent, and platform fee. For Monnify this is the Disbursements (Single Transfer) API, paying out to each recipient's registered bank account and code — no manual bank transfers involved.
5. **On rejection or timeout**, the collecting provider's Refunds API automatically returns the full amount to the tenant — Monnify's Initiate Refund API or Nomba's equivalent, depending on how the tenant paid. This isn't a customer-support escalation path — it's the system's default behavior when verification fails.

Every state transition — held, verification submitted, verified, disbursed, refunded — is driven by the collecting provider's webhooks (`monnify-signature`-verified for Monnify), not by anyone manually updating a status. The tenant's protection is enforced by the payment infrastructure itself, not by a promise from RentVault or goodwill from the other party.

## Why This Is a Real Integration, Not a Feature Bolted On

RentVault has no product without a payment infrastructure partner, and this build showcases Monnify's full collection-to-disbursement lifecycle end to end: **Checkout** (`init-transaction`) is the entry point that gets funds into a held state in the first place; **Single Transfer Disbursements** is the mechanism that performs the split-disbursement that makes escrow meaningful rather than just a holding pen; **Refunds** is what makes the safety promise actually true rather than aspirational marketing copy; **Transaction Verification** backstops confirmation when a webhook is delayed or missed; and **signed webhooks** are the nervous system connecting all of it — every meaningful state change in the product is a direct, real-time reaction to a Monnify (or Nomba, for tenants who choose that option) payment event. Remove any one of these pieces and the core mechanic stops functioning; this is not a payment button attached to an otherwise-independent idea, and it is not a single hard-coded happy path — the provider is chosen per-transaction and the whole state machine (checkout → collection → escrow → verification → disbursement/refund) is exercised identically regardless of which one the tenant picked.

## What's Built

This is a working product, not a concept slide:

- **Public marketing/listing site** (React + Vite): listing search, listing detail, and a checkout flow where the tenant picks Monnify or Nomba and is carried straight into that provider's hosted payment window and back into a live transaction status view — complete with a real-time verification countdown — without them having to remember to check back manually.
...
- **Landlord dashboard** (Next.js): listing management, Monnify and Nomba payout account setup, verification submission.
...
- **Backend** (Express, TypeScript, deployed as a single Docker service on Render): checkout initiation, signature-verified webhook handling, verification logic, split disbursement, timeout scheduling, and a full audit trail of every transaction's lifecycle. Firestore rules still deploy through Firebase; the API itself moved off Cloud Functions to avoid requiring the paid Blaze plan.
- **Data layer**: Firestore for transactions, listings, split configurations, users, and audit logs; Supabase Storage for verification documents and receipts (Firebase Storage would also require the paid Blaze plan); Firebase Auth with role-based access across tenant, landlord, agent, and admin roles, re-checked server-side on every privileged action.
...
**RentVault. Built on Monnify and Nomba. Rent without the gamble.**